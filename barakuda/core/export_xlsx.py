...
def export_ot_results_xlsx(
        ...
        if isinstance(obj, dict):
            for k, v in obj.items():
                if isinstance(v, dict):
                    # Recurse for nested dicts (keeping dotted prefix)
                    _flatten(f"{prefix}{k}.", v, out)
                elif isinstance(v, list):
                     # Serialize list as JSON string
                    out.append((f"{prefix}{k}", json.dumps(v, ensure_ascii=False)))
                else:
                    out.append((f"{prefix}{k}", str(v) if v is not None else ""))
        elif isinstance(obj, list):
             # Should be handled by caller usually, but if top-level is list:
            out.append((prefix[:-1] if prefix.endswith(".") else prefix, json.dumps(obj, ensure_ascii=False)))
        else:
            out.append((prefix[:-1] if prefix.endswith(".") else prefix, str(obj) if obj is not None else ""))

    # The user provided a slightly different implementation of _flatten inside the request than what I just wrote above.
    # I should use exactly what the user provided:
    # def _flatten(prefix: str, obj: Any, out: list[tuple[str, str]]) -> None:
    #     if isinstance(obj, dict):
    ...
    # I will stick to the user's version of _flatten.
